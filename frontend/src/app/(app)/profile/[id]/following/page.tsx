...
function FollowingContent() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.getFollowing(id, 1, 20),
    ])
      .then(([profileData, followingRes]) => {
        setProfile(profileData);
        setFollowing(followingRes.data);
        setTotal(followingRes.total);
      })
      .catch(() => setError(true))
  ...
  async function handleLoadMore() {
    setLoadingMore(true);
    try {
      const res = await api.getFollowing(params.id as string, page + 1, 20);
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-dark-bg flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-6">Could not load following list.</p>
          <button
            onClick={() => router.back()}
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-dark-bg pb-20 lg:pb-0">
      <div className="max-w-2xl mx-auto px-4 pt-4 md:pt-24">
        ...
        <h1 className="text-xl md:text-2xl font-bold mb-4 md:mb-6 dark:text-white">
          {profile ? `Following (${profile._count?.following ?? 0})` : "Following"}
        </h1>
        ...
        {following.length > 0 ? (
          ...
          <h3 className="text-lg font-bold mb-1 dark:text-white">Not following anyone yet</h3>
            <p className="text-sm text-gray-400">When this user follows someone, they&apos;ll appear here.</p>
          </div>
        )}
      </div>
    </div>
